GenerateDescriptors <- function(structures.file, threads = -1, limit = -1) {
...
GenerateDescriptorsFile <- function(structures.file, descriptors.file, threads = -1, limit = -1) {
  standardised.file <- tempfile("standardised", fileext=".sdf")
  name.file <- tempfile("name", fileext=".txt")
  StandardiseMolecules(structures.file, standardised.file, name.file = name.file, limit = limit)
  GenerateDescriptors.internal(standardised.file, descriptors.file, name.file, threads)
...
GenerateDescriptors.internal <- function(structures.file, descriptors.file, name.file, threads = -1) {
  ...
  padel_config_file <- system.file("extdata", "config.txt", package="smpredict")
