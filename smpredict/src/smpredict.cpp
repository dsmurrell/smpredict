// to compile use: PKG_LIBS="-L /usr/local/lib/indigo -lindigo" R CMD SHLIB vecsqr.c
// run 'otool -L vecsqr.so' to see where the executable is pointing to it's dynamin library
// use 'install_name_tool -change libindigo.dylib /usr/local/lib/indigo/libindigo.dylib ./vecsqr.so' to change the path of where vecsqr.so looks for libindigo.dylib
